...
class BenchmarkFamilies {
 public:
  ...
  size_t AddBenchmark(detail::shared_ptr<Benchmark> family);
 ...
 private:
  ...
  std::vector<detail::shared_ptr<Benchmark> > families_;
  Mutex mutex_;
};
...
bool BenchmarkFamilies::FindBenchmarks(
  ...
  MutexLock l(mutex_);
  foreach (detail::shared_ptr<Benchmark>& family, families_) {
    // Family was deleted or benchmark doesn't match
    if (!family) continue;

    if (family->ArgsCnt() == -1) {
      family->Args(std::vector<int>());
    }
    ...
    foreach (std::vector<int> const& args, family->args_) {
      foreach (int num_threads, *thread_counts) {
        ...
        foreach (int arg, args) {
          ...
          if (arg_i < family->arg_names_.size()) {
            const std::string& arg_name = family->arg_names_[arg_i];
            if (!arg_name.empty()) {
              instance.name +=
...
Benchmark* Benchmark::Arg(int x) {
  ...
  args_.push_back(std::vector<int>(1, x));
  return this;
}
...
Benchmark* Benchmark::Range(int start, int limit) {
  ...
  foreach (int i, arglist) { args_.push_back(std::vector<int>(1, i)); }
  return this;
}
...
Benchmark* Benchmark::DenseRange(int start, int limit, int step) {
  ...
  for (int arg = start; arg <= limit; arg += step) {
    args_.push_back(std::vector<int>(1, arg));
  }
  return this;
}
