...
SubMap& GetSubstitutions() {
    ...
    static SubMap map;
    if (map.empty()) {
        map.push_back(std::make_pair("%float", "[0-9]*[.]?[0-9]+([eE][-+][0-9]+)?"));
        map.push_back(std::make_pair("%int", "[ ]*[0-9]+"));
        map.push_back(std::make_pair(" %s ", "[ ]+"));
        map.push_back(std::make_pair("%time", "[ ]*[0-9]{1,5} ns"));
        map.push_back(std::make_pair("%console_report", "[ ]*[0-9]{1,5} ns [ ]*[0-9]{1,5} ns [ ]*[0-9]+"));
        map.push_back(std::make_pair("%csv_report", "[0-9]+," + dec_re + "," + dec_re + ",ns,,,,,"));
    }
    return map;
}
...
struct ReporterTest {
  ...
  ReporterTest(const char* n,
      ...
      : name(n), output_cases(out_tc), error_cases(err_tc), reporter(br),
        out_stream(new std::stringstream), err_stream(new std::stringstream) {
