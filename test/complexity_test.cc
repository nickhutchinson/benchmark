...
int AddComplexityTest(std::string big_o_test_name,
  ...
  ADD_CASES(TC_ConsoleOut, {
    ...
    TestCase("^%bigo_name", MR_Not), // Assert we we didn't only matched a name.
  ...
  ADD_CASES(TC_JSONOut, {
    ...
    TestCase("}", MR_Next),
    TestCase("\"name\": \"%rms_name\",$"),
    TestCase("\"rms\": [0-9]+%$", MR_Next),
    TestCase("}", MR_Next)
  });
  ADD_CASES(TC_CSVOut, {
...
BENCHMARK(BM_Complexity_O_N) -> RangeMultiplier(2) -> Range(1<<10, 1<<16) -> Complexity(oN);
...
BENCHMARK(BM_Complexity_O_N_log_N) -> RangeMultiplier(2) -> Range(1<<10, 1<<16) -> Complexity(oNLogN);
