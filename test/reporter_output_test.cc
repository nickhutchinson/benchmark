...
ADD_CASES(TC_CSVOut, {
    TestCase("name,iterations,real_time,cpu_time,time_unit,bytes_per_second,items_per_second,"
...
ADD_CASES(TC_ConsoleOut, {
   TestCase("^BM_basic %console_report$")
});
ADD_CASES(TC_JSONOut, {
    TestCase("\"name\": \"BM_basic\",$"),
...
BENCHMARK(BM_error);
ADD_CASES(TC_ConsoleOut, {
    TestCase("^BM_error[ ]+ERROR OCCURRED: 'message'$")
});
ADD_CASES(TC_JSONOut, {
    TestCase("\"name\": \"BM_error\",$"),
    TestCase("\"error_occurred\": true,$", MR_Next),
    TestCase("\"error_message\": \"message\",$", MR_Next)
});

ADD_CASES(TC_CSVOut, {
    TestCase("^\"BM_error\",,,,,,,,true,\"message\"$")
});
...
ADD_CASES(TC_JSONOut, {
    TestCase("\"name\": \"BM_RepeatOnce/repeats:1\",$")
});
ADD_CASES(TC_CSVOut, {
    TestCase("^\"BM_RepeatOnce/repeats:1\",%csv_report$")
});
